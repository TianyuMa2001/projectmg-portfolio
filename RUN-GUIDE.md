# ProjectMG - Portfolio Debug Edition

Tianyu Ma - Technical Designer. C# / Unity team course project.

## Windows launch
Extract the entire Windows ZIP and run ProjectMG.exe. Keep ProjectMG_Data, MonoBleedingEdge and UnityPlayer.dll beside it. Windows x64; Unity Editor is not required. Original artwork has been replaced by original abstract debug swatches. This is an unsigned mechanics review build.
...
The source/evidence ZIP contains project scripts, MIT notice, independent .NET checks, and recorded route JSON. It is not a complete Unity project. Open the GitHub project with Unity 6000.3.8f1 for the full original source; original art is not licensed for redistribution. Recreate the public build with the isolated preparation/build scripts after installing dependencies. See RELEASE-STATUS.md for scope.
